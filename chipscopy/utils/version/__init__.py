...
# NOTE: This file will become version.py in the future. It is __init__.py temporarily while
#       some api restructure is done.
# TODO: Move contents of this file to utils/version.py after removing print_versions() function
...
class ServerVersionInfo:
    ...
    def __init__(self, server: ServerInfo, server_type: str):
        ...
        if server_type == "hw_server":
            ...
            self.url = server.url
            version_info_str = cs_service.get_chipscope_server_version_info().args
            version_dict = json.loads(version_info_str)
            self.build = version_dict["build_number"]
            self.version = version_dict["version"]
            self.timestamp = version_dict["timestamp"]
