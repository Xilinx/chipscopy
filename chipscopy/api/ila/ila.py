...
from chipscopy.api.ila.ila_waveform import export_waveform_to_stream, get_waveform_data_values
...
class ILA(DebugCore["AxisIlaCoreClient"]):
    ...
    def __str__(self) -> str:
        ila_dict = {
            "name": self.name,
            "core_info": self.core_info.__dict__,
            "static_info": self.static_info.__dict__,
            "status": self.status.__dict__,
            "control": self.control.__dict__,
            "ports": self.ports,
        }
        return pformat(ila_dict, 2)
    ...
    def upload(self) -> bool:
        ...
        if uploaded:
            wave = self.core_tcf_node.get_property_group(["data"])
            wave["trace_probes"] = self._make_waveform_probes()
    ...
    def monitor_status(
        ...
        **Synchronous Mode**
        ...
        - Typical mode to use this function.
    ...
    def _make_waveform_probes(self) -> {str, ILAWaveformProbe}:
        return {
            p.name: ILAWaveformProbe(p.name, p.map, self._probe_to_port_seqs[p.map])
