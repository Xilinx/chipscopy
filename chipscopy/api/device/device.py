...
from chipscopy.api.memory import DeviceMemory
...
class Device:
    ...
    def __init__(
        ...
        *,
        ...
        """CTOR - Device is created and managed by the Session parent -
        see session.devices

        Args:
            hw_server: hardware server
            cs_server: chipscope server. May be None if no cs_server is connected.
            device_identification: identification routines for this device in the node tree
            disable_core_scan: True=disable setup_debug_cores scanning, False=force the scan (default is False)
        """
    ...
    def __str__(self):
        retval = str(self.device_identification)
        return retval

    def __repr__(self):
        retval = repr(self.device_identification)
        return retval
    ...
    def memory(self):
        ...
        device_memory = DeviceMemory(self.hw_server, self.device_identification)
    ...
    def memory_write(self, address: int, values: List[int], *, size="w", target=None):
        ...
        """
        target_node = self.device_identification.find_memory_target(self.hw_server, target)
        DeviceMemory.memory_write(
            memory_tcf_node=target_node, address=address, values=values, size=size
        )
    ...
    def memory_read(self, address: int, num: int = 1, *, size="w", target=None) -> List[int]:
        ...
        Note: This method should not be used in inner loops. It is not the fastest because it looks up
              the memory target every time. Inner loops should just call the find_memory_target once.
        """
        # FIXME: This code is quick hardcoded for versal memory targets - convenient but not maintainable long term.
        target_node = self.device_identification.find_memory_target(self.hw_server, target)
        retval = DeviceMemory.memory_read(
            memory_tcf_node=target_node, address=address, num=num, size=size
        )
        return retval
