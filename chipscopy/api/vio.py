...
class VIOProbe:
    ...
    def __init__(self, ltx_probe: LtxProbe):
        ...
        self.bus_left_index = ltx_probe.msb_index
        self.bus_right_index = ltx_probe.lsb_index
...
class VIO(DebugCore["AxisVIOCoreClient"]):
    ...
    def read_ports(self, port_names: Union[str, List[str]] = None):
        ...
        Args:
            port_names: **(optional)** List of port names (see
                ``vio_port_names``). The default is all ports if no ports are
                specified.
    ...
    def write_ports(self, port_values: Dict[str, int]):
        ...
        the VIO convention port_in# and port_out#. See ``vio_port_names``.
        ...
        Args:
            port_values: dict. key=port_name, value=port_value.
                ...
                ::
                      ...
                      write_vio_ports({'probe_out0': 1000,
                                       'probe_out1': 0x100})
    ...
    def probe_names(self):
        """
        Returns:
             list(str): List of probe names from the LTX file
        """
        probe_names = []
        if self.ltx_core is not None:
            for probe in self.ltx_core.probes:
                probe_names.append(probe.name)
        return probe_names

    def read_probes(self, probe_names: List[str] or str = None):
        """Read probe values from hardware. Gets the current values from
        hardware for selected input and output probes.

        Args:
            probe_names: List of probe names (see ``vio_probe_names``).
        ...
        Returns:
            dict:
                ...
                probe_data returned as a dictionary in the following format:
    ...
    @staticmethod
    def _build_probe_to_port_map(ltx_probes: [LtxProbe]):
        # Build the probe to port mapping. This mapping associates the logical
    ...
    def _create_probes(self, probe_to_port_map: Dict[int, list]):
    ...
    def _setup_vio_from_ltx(self, ltx: Ltx):
        ...
        # TODO: Raise appropriate exception if the ltx does not match and matching core is not found
        assert ltx_core is not None
        self.instance_name = ltx_core.cell_name
        self.name = ltx_core.cell_name
        probe_to_port_map = self._build_probe_to_port_map(ltx_core.probes)
