...
class EyeScanPlot:
    ...
    def _generate(self):
        ...
        for y in self._y:
            ...
            for x in self._x:
                val = log(self.eye_scan.scan_data.processed.scan_points[(x, y)].ber)
                z_hovertext_row.append(format(exp(val), ".2e"))
                row_vals.append(val)
        ...
        colorbar_number_of_ticks: int = 8
        ...
        for x_factor in range(colorbar_number_of_ticks):
            ...
            color_bar_tick_text.append(format(exp(val), ".2e"))
