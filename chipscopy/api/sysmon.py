...
class Sysmon(DebugCore["SysMonCoreClient"]):
    ...
    It wraps the old client sysmon_core_client.SysMonCoreClient(CoreClient) class
    ...
    def refresh_property_group(self, groups: List[str], done: DoneHWCommand = None) -> List[str]:
        ...
        Args:
            groups (List[str]): list of property group names. For SysMon the groups are ``control``, ``status``,
                and ``sensor``, ``temp``, and ``vccint``.
