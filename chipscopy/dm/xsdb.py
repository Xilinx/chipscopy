...
def add_manager_from_channel(channel):
    serv = channel.getRemoteService(dc_service.XsdbService)
    if not serv:
        raise Exception("Server does not have Xsdb Service")
    ...
    cs_manager = XsdbManager(name, channel)
...
class XsdbManager(CsManager):
    def __init__(self, name: str = "", channel=None):
        super(XsdbManager, self).__init__(name, channel)
        self.dc = None
        self.poll_scheduler = None
        if self.channel:
            self.dc = self.channel.getRemoteService(dc_service.XsdbService)
            # self.poll_scheduler = DebugCorePollScheduler(self.channel)
        if self.dc:
            self.dc.add_listener(XsdbNodeListener(self))
...
class ChannelOpenListener(protocol.ChannelOpenListener):
    def onChannelOpen(self, channel):
        if channel.getRemoteService(dc_service.XsdbService):
            add_manager_from_channel(channel)


class ManagerChannelListener(ChannelListener):
    def __init__(self, cs_manager: XsdbManager):
